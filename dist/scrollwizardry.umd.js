(function (global, factory) {
	...
	var Util = function () {
	  ...
	  createClass(Util, null, [{
	    ...
	    value: function elements() {
	      ...
	      if (lodash.isElement(selector) || selector === document || selector === window) {
	        return [selector];
	      }
	      return selector;
	    }
	    ...
	    value: function css(el, _css) {
	      ...
	      lodash.forEach(_css, function (value, key) {
	        if (value === parseFloat(value)) {
	          // assume pixel for seemingly numerical values
	          value += 'px';
	        }
	        el.style[lodash.camelCase(key)] = value;
	      });
	...
	var Indicator = function () {
	  function Indicator(scene, options) {
	    ...
	    options = lodash.merge({}, DEFAULT_INDICATOR_OPTIONS, options);
	...
	var NAMESPACE = 'ScrollWizardry.Scene';
	...
	var Scene = function () {
	  function Scene(options) {
	    ...
	    this.options = lodash.merge({}, DEFAULT_SCENE_OPTIONS, options);
	    ...
	    this.validate = {
	      duration: function duration(val) {
	        ...
	        if (lodash.isString(val) && val.match(/^(\.|\d)*\d+%$/)) {
	          // percentage value
	      ...
	      loglevel: function loglevel(val) {
	        val = parseInt(val, 10);
	        if (!lodash.isNumber(val) || val < 0 || val > 3) {
	  ...
	  createClass(Scene, [{
	    ...
	    value: function _updateScrollOffset() {
	      var offset = lodash.isFunction(this.options.offset) ? this.options.offset() : this.options.offset;
	      ...
	      check.forEach(function (optionName) {
	        ...
	        if (_this7.validate[optionName]) {
	          ...
	          try {
	            ...
	            var logMSG = lodash.isString(event) ? [event] : event;
	            if (lodash.isArray(logMSG)) {
	    ...
	    value: function setPin(element, settings) {
	      ...
	      settings = lodash.merge({}, defaultSettings, settings);
	      ...
	      var pinCSS = lodash.pick(Util.css(this._pin), boundsParams.concat(['display']));
	      var sizeCSS = lodash.pick(Util.css(this._pin), ['width', 'height']);
	      ...
	      var spacerCSS = lodash.merge(pinCSS, {
	      ...
	      if (!inFlow) {
	        ...
	        lodash.merge(spacerCSS, sizeCSS);
	      }
	    ...
	    value: function setClassToggle(element, classes) {
	      ...
	      if (els.length === 0 || !lodash.isString(classes)) {
	...
	var NAMESPACE$1 = 'ScrollWizardry.Controller';
	...
	var Controller = function () {
	  function Controller(options) {
	    ...
	    this.options = lodash.merge({}, DEFAULT_CONTROLLER_OPTIONS, options);

	    this.options.container = Util.elements(this.options.container)[0];

	    if (!this.options.container) {
	      Log.log(1, 'ERROR creating object ' + NAMESPACE$1 + ': No valid scroll container supplied');
	    ...
	    this.options.refreshInterval = lodash.isNumber(ri) ? ri : DEFAULT_CONTROLLER_OPTIONS.refreshInterval;
	  ...
	  createClass(Controller, [{
	    ...
	    value: function _updateScenes() {
	      ...
	      if (this._enabled && this._updateScenesOnNextCycle) {
	        ...
	        var scenesToUpdate = lodash.isArray(this._updateScenesOnNextCycle) ? this._updateScenesOnNextCycle : this._sceneObjects.slice(0);
	    ...
	    value: function scrollPos(scrollPosMethod) {
	      ...
	      if (lodash.isFunction(scrollPosMethod)) {
	    ...
	    value: function destroy(resetScenes) {
	      ...
	      Log.log(3, 'destroyed ' + NAMESPACE$1 + ' (reset: ' + (resetScenes ? 'true' : 'false') + ')');
	    ...
	    value: function updateBoundsPositions(specificIndicator) {
	      ...
	      var groups = specificIndicator ? [lodash.merge({}, specificIndicator.triggerGroup, { members: [specificIndicator] })] : // create a group with only one element
